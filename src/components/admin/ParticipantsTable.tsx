...
export function ParticipantsTable({ participants, onRefresh }: ParticipantsTableProps) {
  ...
  const filteredAndSortedParticipants = participants
    .filter(participant => {
      const riskMatch = filterRisk === 'all' || participant.riskLevel === filterRisk;
      const categoryMatch = filterCategory === 'all' || participant.category === filterCategory;
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredAndSortedParticipants.map((participant, index) => (
              <tr key={index} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm text-gray-900">{participant.category}</div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm font-semibold text-gray-900">{participant.totalVotes}</div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className={`text-sm ${getIncognitoColor(participant.incognitoPercentage)}`}>
                    {participant.incognitoPercentage.toFixed(1)}%
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm text-gray-900">{participant.incognitoVotes}</div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-xs">
                    {participant.suspiciousIPs.slice(0, 2).map((ip, idx) => (
                      <div key={idx} className="font-mono text-blue-600">{ip}</div>
                    ))}
                    {participant.suspiciousIPs.length > 2 && (
                      <div className="text-gray-500">+{participant.suspiciousIPs.length - 2} more</div>
                    )}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getRiskColor(participant.riskLevel)}`}>
